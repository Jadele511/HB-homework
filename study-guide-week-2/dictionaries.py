...
def get_sum_zero_pairs(numbers):
    ...
    for i in range(len(numbers)):
        for j in range(len(numbers)):
            ...
            num_j = numbers[j]
            sub = [num_i, num_j]
            negative_sub = [-num_i, -num_j]
            if num_i + num_j == 0 and sub not in res and negative_sub not in res:
                res.append(sub)
    return res
...
def top_chars(phrase):
    ...
    max_char = char_count_list[0][0]
    max_char_count = char_count_list[0][1]
    res = [max_char]
    for i in range(1, len(char_count_list)):
        if char_count_list[i][1] == max_char_count:
            res.append(char_count_list[i][0])
    return res
